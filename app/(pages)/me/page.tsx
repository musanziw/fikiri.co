...
export default function Solution() {
  ...
  return (
    <div className={"relative"}>
      ...
      {user ? (
        <div
          className={`p-8 max-w-screen-sm mx-auto flex flex-col pt-20 border-x border-dashed`}
        >
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-3 mb-8">
              {LINKS.map((link, index) => (
                <button
                  key={index}
                  className={`flex items-center gap-2 py-2 px-4 rounded-md cursor-pointer ${
                    link.index === active
                      ? "bg-gray-300/70 text-gray-950"
                      : "bg-gray-100/90"
                  }`}
