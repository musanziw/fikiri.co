            ...
            <div className="flex flex-col gap-2 ">
                <div className="flex flex-col gap-2 mb-4">
                    <h2 className={'font-bold text-lg'}>
                        {solution.name} <Link href={`/solutions/update/${solution.id}`}><Pencil
                        className={'inline-block ml-2'} size={16}/></Link>
                    </h2>
                ...
                <div className={`grid grid-cols-3 gap-3`}>
                    {
                        solution.images?.length > 0 && solution.images.map((image: any, index: number) => (
                            <div key={index}>
                                <Image src={`${apiBaseURL}uploads/${image.imageLink}`} alt={solution.name}
